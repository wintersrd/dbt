    ...
    def _link_cached_relations(self, schemas):
        try:
            _, table = self.execute(
                sql=_LIST_RELATIONS_SQL,
                model_name=GET_RELATIONS_OPERATION_NAME,
                auto_begin=True,
                fetch=True
            )
        finally:
...
from relationships
join schema as dependent_schema on
    relationships.dependent_schema_id=dependent_schema.id
join schema as referenced_schema on
    relationships.referenced_schema_id=referenced_schema.id
