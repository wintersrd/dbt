...
def handle_and_check(args):
    with log_manager.applicationbound():
        ...
        with dbt.profiler.profiler(enable=profiler_enabled, outfile=parsed.record_timing_info):
...
def track_run(task):
    ...
    except (dbt.exceptions.NotImplementedException, dbt.exceptions.FailedToConnectException) as e:
...
def _build_base_subparser():
    ...
    base_subparser.add_argument("--log-cache-events", action="store_true", help=argparse.SUPPRESS)
...
def _build_source_snapshot_freshness_subparser(subparsers, base_subparser):
    ...
    sub.set_defaults(cls=freshness_task.FreshnessTask, which="snapshot-freshness", rpc_method=None)
    return sub
...
def parse_args(args, cls=DBTArgumentParser):
    ...
    _add_common_arguments(run_sub, compile_sub, generate_sub, test_sub, rpc_sub, seed_sub)
    # --models, --exclude
